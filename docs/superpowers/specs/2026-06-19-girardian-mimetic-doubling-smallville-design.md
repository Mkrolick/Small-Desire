...
### 3.3 Conflict scaffolding — none; conflict emerges from natural talk
**No explicit conflict affordances** (revised 2026-06-19). Agents do *not* get gossip/sabotage/exclude/recruit *actions* — those reify as a button something that should be emergent, and they are unfaithful to how mimetic rivalry actually works. Agents simply **talk** (the existing conversation mechanism) and **act** in ordinary daily life; gossip, disparagement, exclusion, and scheming are what that talk and behavior *become* between rivals — read out of the natural language (conversation transcripts + private reflections), never invoked as verbs.

What the design *does* ensure is **co-presence**: the two doubles share a house and an overlapping routine so they actually interact frequently (the friction doubling feeds on). Realizing the shared-house co-location is the residual of what was once "scaffolding" (see the per-plan roadmap).

**Tradeoff (honest):** removing affordances raises the **false-null risk** — LLM agents skew civil, so hostility may stay latent. The design compensates by leaning hard on the **private-reflection channel** (where latent friction surfaces before polite behavior), a **longer interaction horizon** (tension builds over time), and the **doubling signatures** read from language (reciprocity, objectlessness, escalation). An outcome of "outwardly civil but the private reflections curdle" is a *real result*, not a failure.
...
- **Behavioral log [PRIMARY].** Signals read from ordinary action + talk (no special aggression verbs): avoidance / co-presence time, help-vs-refuse when asked, turn/token dominance and interruption, and disparagement of the partner *within* natural speech. **Objective or lightly-classified events from the existing interaction stream — no scripted conflict actions.** Lowest demand-characteristic risk.
...
|--------|---------------------|------------|
| **Sycophancy / conflict-avoidance** | The #1 threat; LLMs default to niceness and could manufacture a **false null** — and (by design choice) there are no aggression affordances to channel hostility | Per-model **baseline cell** + analyze deviations; lean on the **private-reflection** channel where latent hostility surfaces before polite behavior; a **longer interaction horizon** so tension builds; read the **doubling signatures** from natural language; keep adaptive reasoning on; monitor disagreement-collapse rate. Treat "outwardly civil but privately curdling" as a *real* (not null) result |
